package com.sulake.core.window.utils
{
   ...
   public class WindowMouseOperator implements IDisposable
   {
      ...
      protected var var_535:Point;
      
      protected var var_116:DisplayObject;
      ...
      protected function handler(param1:Event) : void
      {
         ...
         if(_working)
         {
            if(param1.type == Event.ENTER_FRAME)
            {
               ...
               {
                  operate(var_116.mouseX,var_116.mouseY);
      ...
      public function operate(param1:int, param2:int) : void
      {
         _mouse.x = param1;
         _mouse.y = param2;
         getMousePositionRelativeTo(_window,_mouse,var_535);
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         if(_working)
         {
            if(_window == param1)
            {
               var_116.removeEventListener(MouseEvent.MOUSE_DOWN,handler,false);
               var_116.removeEventListener(MouseEvent.MOUSE_UP,handler,false);
               var_116.removeEventListener(Event.ENTER_FRAME,handler);
               if(true)
               {
                  _window.removeEventListener(WindowEvent.const_401,clientWindowDestroyed);
               }
               _window = null;
               _working = false;
            }
      ...
      public function begin(param1:IWindow, param2:uint = 0) : IWindow
      {
         ...
         {
            var_116.addEventListener(MouseEvent.MOUSE_DOWN,handler,false);
            var_116.addEventListener(MouseEvent.MOUSE_UP,handler,false);
            var_116.addEventListener(Event.ENTER_FRAME,handler);
            _mouse.x = var_116.mouseX;
            _mouse.y = var_116.mouseY;
            _window = WindowController(param1);
            getMousePositionRelativeTo(param1,_mouse,_offset);
            _window.addEventListener(WindowEvent.const_401,clientWindowDestroyed);
            _working = true;
         }
         return _loc3_;
      }
