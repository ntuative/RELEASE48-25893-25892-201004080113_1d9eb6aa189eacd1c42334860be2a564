package com.sulake.core.window.utils
{
   ...
   public class WindowParser implements IWindowParser
   {
      ...
      public static const const_1553:String = "_INCLUDE";
      
      public static const const_984:String = "_EXCLUDE";
      
      public static const const_1571:String = "_TEMP";
      
      private static const const_1086:RegExp = /^(\s|\n|\r|\t|\v)*/m;
      
      private static const const_1087:RegExp = /(\s|\n|\r|\t|\v)*$/;
       
      
      protected var var_1108:Dictionary;
      
      private var _disposed:Boolean = false;
      
      protected var var_940:Dictionary;
      
      protected var var_1386:Map;
      
      protected var var_14:IWindowContext;
      
      protected var var_1385:Dictionary;
      
      protected var var_1109:Dictionary;
      ...
      public function WindowParser(param1:IWindowContext)
      {
         ...
         var_1108 = new Dictionary();
         TypeCodeTable.fillTables(var_1109,var_1108);
         var_940 = new Dictionary();
         var_1385 = new Dictionary();
         ParamCodeTable.fillTables(var_940,var_1385);
         var_1386 = new Map();
      }
      
      private static function trimWhiteSpace(param1:String) : String
      {
         param1 = param1.replace(const_1087,"");
         return param1.replace(const_1086,"");
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            for(_loc1_ in var_1109)
            {
               delete var_1109[_loc1_];
            }
            for(_loc1_ in var_1108)
            {
               delete var_1108[_loc1_];
            }
            for(_loc1_ in var_940)
            {
               delete var_940[_loc1_];
            }
            for(_loc1_ in var_1385)
            {
               delete var_1385[_loc1_];
            }
            var_1386.dispose();
            var_1386 = null;
            var_14 = null;
      ...
      private function parseSingleWindowEntity(param1:XML, param2:WindowController, param3:Map) : IWindow
      {
         ...
         window = var_14.create(name,null,type,style,params,rect,null,parent is IIterable ? null : parent,id,parseProperties(xml.child("variables")[0]),tags) as WindowController;
            ...
            if(parent != null)
            {
               if(parent is IIterable)
               {
                  if(window.x != rect.x || window.y != rect.y || window.width != rect.width || window.height != rect.height)
                  {
                     ...
                     if((params & 0) == WindowParam.const_158)
                     {
                        window.y = rect.y;
                     }
                  }
         ...
         if(_loc7_ is IIterable)
         {
            ...
            if(_loc9_ > 0)
            {
               ...
               while(_loc12_ < _loc9_)
               {
                  ...
                  if(_loc10_.tags.indexOf(WindowParser.const_984) == -1)
                  {
