package com.sulake.core.runtime
{
   ...
   public class ComponentContext extends Component implements IContext
   {
      ...
      protected var var_243:Array;
      
      protected var var_372:Array;
      
      protected var _debug:Boolean = false;
      
      protected var var_288:Array;
      ...
      protected function removeLibraryLoader(param1:LibraryLoader) : void
      {
         ...
         {
            if(var_243[_loc2_] == param1)
            {
               var_243.splice(_loc2_,1);
               break;
            }
            _loc2_++;
         }
      ...
      protected final function announceInterfaceAvailability(param1:IID, param2:Component) : void
      {
         ...
         while(_loc6_ < _loc5_)
         {
            ...
            if(_loc4_ == null)
            {
               error("Interface " + param1 + " still unavailabe!",true,Core.const_1271);
            }
      ...
      public final function loadFromFile(param1:URLRequest, param2:LoaderContext) : LibraryLoader
      {
         ...
         var _loc3_:int = 0;
         while(_loc3_ < var_243.length)
         {
            if(var_243[_loc3_].url == param1.url)
            {
               return var_243[_loc3_].loadFromFile(param1);
            }
            _loc3_++;
         }
         var _loc4_:LibraryLoader = new LibraryLoader(_debug);
      ...
      override public function toXMLString(param1:uint = 0) : String
      {
         ...
         var _loc8_:uint = var_180.mapStructsByImplementor(this,_loc7_);
         var _loc9_:int = 0;
         while(_loc9_ < _loc8_)
         {
         ...
         var _loc11_:int = 0;
         while(_loc11_ < var_288.length)
         {
            _loc10_ = var_288[_loc11_] as Component;
            if(_loc10_ != this)
            {
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            if(var_372 != null)
            {
               ...
               while(_loc1_-- > 0)
               {
                  IDisposable(var_372.pop()).dispose();
               }
               var_372 = null;
            }
            if(var_243 != null)
      ...
      protected final function hasQueueForInterface(param1:IID) : Boolean
      {
         ...
         while(_loc4_ < _loc3_)
         {
            if(getQualifiedClassName(var_372[_loc4_].identifier) == _loc2_)
            {
               return true;
            }
            _loc4_++;
      ...
      protected function loadErrorHandler(param1:LibraryLoaderEvent) : void
      {
         ...
         removeLibraryLoader(_loc2_);
         error("Failed to download component resource \"" + _loc2_.url + "\"!" + "\r" + _loc3_,true,Core.const_1151);
      }
      
      public final function detachComponent(param1:Component) : void
      {
         var _loc2_:* = null;
         var _loc3_:int = var_180.getIndexByImplementor(param1);
         while(_loc3_ > -1)
         {
            _loc2_ = var_180.remove(_loc3_);
            _loc3_ = var_180.getIndexByImplementor(param1);
         }
         var _loc4_:int = 0;
         while(_loc4_ < var_288.length)
         {
            if(var_288[_loc4_] == param1)
            {
               var_288.splice(_loc4_,1);
               return;
            }
            _loc4_++;
         }
         ...
         while(i < componentList.length())
         {
            ...
            if(assetCount > 0)
            {
               ...
               while(k < assetCount)
               {
                  ...
                  {
                     error("Manifest for component " + componentString + " contains broken asset \"" + assetName + "\"!",true,Core.const_679);
                  }
                  k++;
               }
            }
            componentString = item.attribute("HabboNavigatorCom_grs_official_room_row_xml");
            ...
            if(componentClass == null)
            {
               error("Invalid component class " + componentString + "!",true,Core.const_679);
               return false;
            }
            ...
            if(component != null)
            {
               if(assetLibrary != null)
               {
                  if(component.assets != assetLibrary)
                  {
                     ...
                     error("Component \"" + componentString + "\" did not save provided asset library!",true,Core.const_679);
                  }
               }
      ...
      {
         var _loc3_:InterfaceStruct = var_180.getStructByInterface(param1);
         if(_loc3_ != null)
         {
      ...
      public final function attachComponent(param1:Component, param2:Array) : void
      {
         ...
         while(_loc3_ < _loc4_)
         {
            ...
            var_180.insert(new InterfaceStruct(_loc5_,param1));
            _loc3_++;
         }
         _loc3_ = 0;
         while(_loc3_ < _loc4_)
