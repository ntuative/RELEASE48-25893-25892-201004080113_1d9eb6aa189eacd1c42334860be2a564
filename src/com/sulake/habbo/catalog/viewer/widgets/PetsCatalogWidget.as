package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_437 >= var_313.length)
         {
            return "";
         }
         var petColor:IPetColor = var_313[var_437] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_609[var_1574] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_437 = param1.index;
         if(var_437 < 0 || var_437 > var_313.length)
         {
            var_437 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_548.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_143,onTextWindowEvent);
         var_548 = new Map();
         ...
         var_909 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_313 = [];
         var_609 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_909);
               if(_loc11_)
               {
                  var_609 = _loc11_.breeds;
                  var_313 = _loc11_.colors;
                  if(var_313 == null)
                  {
                     var_313 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_609.length)
         {
            _loc12_ = var_609[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_909,_loc12_.id),getRaceLocalizationKey(var_909,_loc12_.id)));
            _loc6_++;
         }
         var_548.add(var_909,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_908,onApproveNameResult);
         _loc5_ = var_548.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_313)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_548.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_437 >= 0 && var_437 < var_313.length)
         {
            _loc7_ = var_313[var_437] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_609[var_1574] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_909,_loc3_.id,_loc2_,AvatarScaleType.const_51);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_297,AvatarAction.const_699);
         _loc4_.endActionAppends();
         _loc4_.setDirection(AvatarSetType.const_38,2);
         setPreviewImage(_loc4_.getCroppedImage(AvatarSetType.const_38).clone(),true);
         _loc4_.dispose();
